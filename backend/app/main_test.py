"""FastAPI backend for acoustic room simulator - TEST VERSION.

This version doesn't immediately start background processing to allow testing
of job management without simulation complexity.
"""
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Acoustic Room Simulator backend (TEST MODE)...")
    ...
    logger.info("Backend startup complete (TEST MODE)")
...
app = FastAPI(
    title="Acoustic Room Simulator (TEST)",
    description="FEM-based acoustic simulation with geometric acoustics - TEST MODE",
    version="0.1.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint."""
    return {"message": "Acoustic Room Simulator API (TEST MODE)", "version": "0.1.0"}
...
async def submit_simulation(
    ...
    """
    Submit a new simulation job (TEST MODE - no background processing).
    ...
    try:
        ...
        job_status = JobStatus(
            ...
            status="pending",
            progress=0.0,
            message="Job submitted (TEST MODE - no processing)",
        ...
        logger.info(f"Submitted simulation job {job_id} (TEST MODE)")
        return job_status
...
async def start_simulation(job_id: str, background_tasks: BackgroundTasks):
    """
    Start processing for a specific job (TEST MODE manual trigger).
    """
    try:
        global job_manager
...
async def process_simulation_job(job_id: str, request: SimulationRequest):
    """Process a simulation job in the background (TEST MODE)."""
    global job_manager, results_io
    
    try:
        logger.info(f"Starting simulation job {job_id} (TEST MODE)")
        
        # Update job status
        await job_manager.update_job_status(
            job_id, "running", 0.0, "Initializing simulation (TEST MODE)"
        )
        ...
        await job_manager.update_job_status(
            job_id, "running", 0.5, "Processing simulation (TEST MODE)"
        )
        ...
        # Update job status
        await job_manager.update_job_status(
            job_id, "completed", 1.0, "Simulation completed (TEST MODE)"
        )
        
        logger.info(f"Completed simulation job {job_id} (TEST MODE)")
